        ...
        public void CanBuildUsingLatestDotNetCorePackages()
        {
            using (var fs = new DisposableFileSystem())
            {
                ...
                var compileResult = WixRunner.Execute(new[]
                {
                    ...
                    "-ext", TestData.Get(@"ImeSense.WixToolset.BootstrapperApplications.wixext.dll").Replace("NetFx.wixext", "Bal.wixext"),
                    "-ext", TestData.Get(@"ImeSense.WixToolset.Netfx.wixext.dll"),
        ...
        public void CanBuildUsingNetFxSearches()
        {
            using (var fs = new DisposableFileSystem())
            {
                ...
                var compileResult = WixRunner.Execute(new[]
                {
                    ...
                    Path.Combine(bundleSourceFolder, "BundleLatest.wxs"),
                    "-ext", TestData.Get(@"ImeSense.WixToolset.BootstrapperApplications.wixext.dll").Replace("NetFx.wixext", "Bal.wixext"),
                    "-ext", TestData.Get(@"ImeSense.WixToolset.Util.wixext.dll").Replace("NetFx.wixext", "Util.wixext"),
                    "-ext", TestData.Get(@"ImeSense.WixToolset.Netfx.wixext.dll"),
                    "-intermediateFolder", intermediateFolder,
                    "-o", bundleFile,
                    "-arch", "x64",
